...
export const CalendarProvider = ({ children }: CalendarProviderProps) => {
	...
	// Language provider is a parent of this provider
	// So we can use this
	...
	useEffect(() => {
		async function loadCalendar() {
			...
			setError(null);
			try {
				const calendars = await getCalendars(languageCode);
				...
				const defaultCalendar =
					...
					calendars.find(
						(c: Calendar) => c.code === "ROMAN_GENERAL",
					) ??
					calendars[0];
	...
	return (
		<CalendarContext.Provider
			value={{ calendar, setCalendar, isLoading, error }}
		>
			{children}
		</CalendarContext.Provider>
	);
